...
String createOLAPview = "CREATE OR REPLACE VIEW OLAP_DATA AS SELECT s.sensor_id, EXTRACT (YEAR from s.date_created) AS Year, to_char(s.date_created, 'Q') AS Quarter, EXTRACT (MONTH FROM s.date_created) AS Month, ABS(TO_NUMBER(TO_CHAR(s.date_created+1, 'iw')) - TO_NUMBER(TO_CHAR(TRUNC(s.date_created+1, 'mm'), 'iw'))) + 1 AS Week, to_char(s.date_created, 'D') AS Day, AVG(s.value) AS Average, MIN(s.value) AS Minimum, MAX(s.value) AS Maximum FROM scalar_data s WHERE s.sensor_id IN (SELECT sensor_id FROM subscriptions WHERE person_id = 7777) AND s.sensor_id = '21' GROUP BY s.sensor_id, ROLLUP(EXTRACT (YEAR from s.date_created), to_char(s.date_created, 'Q'), EXTRACT (MONTH FROM s.date_created), ABS(TO_NUMBER (TO_CHAR(s.date_created +1 , 'iw') ) - TO_NUMBER(TO_CHAR(TRUNC(s.date_created +1 , 'mm'), 'iw'))) +1 ,to_char(s.date_created, 'D'))";
